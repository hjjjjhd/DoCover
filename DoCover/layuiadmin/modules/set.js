...
layui.define(['form', 'upload'], function (exports) {
    ...
    form.verify({
        ...
        //if(value == undefined || value == '' || value.lenth == 0) return;
        , user_qq: function (value, item) {
            if (!/^[1-9][0-9]{4,}$/.test(value)) {
                return 'QQ号码输入格式不正确';
            }
        }
        , user_phone: function (value, item) {
    ...
    form.on('submit(setmypass)', function (obj) {
        $.ajax({
            url: "/Set/UpdatePassword",
            type: "post",
            data: { oldPassword: obj.field.oldPassword, newPassword: obj.field.repassword },
            success: function (res) {
                res = eval('(' + res + ')');
                if (res.code == 200)
                    layer.msg('更新密码成功');
